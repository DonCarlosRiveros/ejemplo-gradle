pipeline
{
    ...
    post
    {
        success
        {
            slackSend color: "good", channel: "Carlos M. Riveros C.", message: "[Carlos M. Riveros C.] [${env.JOB_NAME}] [$params.opciones] - Ejecución exitosa"
        }
        failure
        {
            slackSend color: "danger", channel: "Carlos M. Riveros C.", message: "[Carlos M. Riveros C.] [${env.JOB_NAME}] [$params.opciones] - Ejecución fallida en stage ${env.STAGE_NAME}"
        }
    }
}
