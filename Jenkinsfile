pipeline
{
    ...
    post
    {
        success
        {
            slackSend color: "good", message: "[Carlos M. Riveros C.] [${env.JOB_NAME}] - Ejecución exitosa"
        }
        failure
        {
            slackSend color: "danger", message: "[Carlos M. Riveros C.] [${env.JOB_NAME}] - Ejecución fallida en stage ${env.STAGE_NAME}"
        }
    }
}
